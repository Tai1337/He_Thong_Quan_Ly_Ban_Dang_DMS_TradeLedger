...
import { findSalesOrdersByDistributor, getTotalAvailableStock } from '../repositories/salesOrderRepository.js';

/**
 * Lấy dữ liệu cho RPT005: Báo cáo đơn hàng thiếu tồn kho
 * Hỗ trợ 2 chế độ: mode='missing_only' | 'all'
 */
export const getRpt005Data = async (distributorId, filters = {}) => {
 ...
 */
export const getRpt057Data = async (distributorId, filters = {}) => {
  const { orders } = await findSalesOrdersByDistributor(distributorId, {
    ...filters,
    page: 1,
    limit: 2000
 ...
 */
export const getRpt006Data = async (distributorId, filters = {}) => {
  const { orders } = await findSalesOrdersByDistributor(distributorId, {
    ...filters,
    page: 1,
    limit: 2000
 ...
 */
export const getRpt061Data = async (distributorId, filters = {}) => {
  const { orders } = await findSalesOrdersByDistributor(distributorId, {
    ...filters,
    page: 1,
    limit: 2000
  ...
  for (const order of orders) {
    for (const item of (order.items || [])) {
      ...
      const lotsStr = (item.allocations || [])
        .map(a => `${a.stockLot?.lotNumber || 'N/A'} (${a.quantity})`)
        .join(', ') || 'Chưa phân bổ';
 ...
 */
export const exportReportToExcel = async ({ title, headers, data, fileName }) => {
